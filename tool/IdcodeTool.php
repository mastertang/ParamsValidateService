...
class IdcodeTool
{
    ...
    {
        if (strlen($idcode) != 15 && strlen($idcode) != 18) {
            return false;
        }
        // 是数值
        if (is_numeric($idcode)) {
            ...
            if (strlen($idcode) == 15) {
                // 省市县（6位）
                $areaNum = substr($idcode, 0, 6);
                // 出生年月（6位）
                $dateNum = substr($idcode, 6, 6);
                // 性别（3位）
                $sexNum = substr($idcode, 12, 3);
            } else {
                // 如果是18位身份证号
                // 省市县（6位）
                $areaNum = substr($idcode, 0, 6);
                // 出生年月（8位）
                $dateNum = substr($idcode, 6, 8);
                // 性别（3位）
                $sexNum = substr($idcode, 14, 3);
                // 校验码（1位）
                $endNum = substr($idcode, 17, 1);
            }
        } else {
            // 不是数值
            if (strlen($idcode) == 15) {
        ...
        if (isset($endNum)) {
            if (!self::checkEnd($endNum, $idcode)) {
                return false;
            }
        }
        return true;
